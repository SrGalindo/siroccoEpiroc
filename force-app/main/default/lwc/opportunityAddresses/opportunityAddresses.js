...
export default class OpportunityAddresses extends LightningElement {
  @api
  recordId
  @api
  ...
  connectedCallback(e) {
    ...
    this.loading = true;
    getPreselectedAddress({ objectApiName: 'Opportunity', recordId: this.recordId, type: 'Ship-To' })
    .then((result) => {
    ...
    getPreselectedAddress({ objectApiName: 'Opportunity', recordId: this.recordId , type: 'Bill-To' })
    .then((result) => {
      if (result && result.length) {
        //this.noRecords = true;
  ...
  addToSelected(event) {
    ...
    this.records.map(record => {
      if (record.Id == selectedId) {
            this.selectedRecord = record;
            this.showPill = true;
            saveAddress({ objectApiName: 'Opportunity', recordId: this.recordId, addressId: record.Id, type: 'Ship-To' })
            .then((result) => {
                const toastEvent = new ShowToastEvent({
                    "variant": "Success",
                    title: 'Success',
                    message: 'Opportunity Bill-to updated',
                  });
                  this.dispatchEvent(toastEvent);
            })
            .catch((ex) => {
  ...
  addToSelected2(event) {
    ...
    console.log('addToSelected5');
      ...
      saveAddress({ objectApiName: 'Opportunity', recordId: this.recordId, addressId: this.selectedRecord2.Id, type: 'Bill-To' })
      .then((result) => {
          this.loading2 = false;
          const toastEvent = new ShowToastEvent({
              "variant": "Success",
